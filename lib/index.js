...
exports.init = function (sequelize, optionsArg) {
  ...
  function createBeforeHook(operation) {
    var beforeHook = function beforeHook(instance, opt) {
      ...
      // Necessary to save the RevisionId of the instance, this is a workaround
      // for https://github.com/sequelize/sequelize/issues/9318 which will not
      // be fixed in v4 of sequelize https://github.com/sequelize/sequelize/pull/9319#issuecomment-383029902
      if (destroyOperation) {
        instance.save({hooks: false})
      }
      if (debug) {
  ...
  function createAfterHook(operation) {
    var afterHook = function afterHook(instance, opt) {
      if (debug) {
        ...
        log('opt:', opt);
        if (ns) {
          log(`CLS ${options.continuationKey}:`, (ns.get(options.continuationKey) || opt[options.continuationKey]));
        }
      }
      ...
      if (instance.context && (instance.context.delta && instance.context.delta.length > 0 || destroyOperation)) {
        ...
        if (failHard && !(ns.get(options.continuationKey) || opt[options.continuationKey])) {
        ...
        query[options.continuationKey] = ns && ns.get(options.continuationKey) || opt[options.continuationKey]
